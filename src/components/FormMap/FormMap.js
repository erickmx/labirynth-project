...
const ColorPicker = ({
  onClick, color, displayColorPicker, onClose, onChange
}) => {
  ...
  return (
    <div>
      ...
      {displayColorPicker ? <div className="popover">
        ...
        <ChromePicker color={color} onChange={(colorSelected) => onChange(colorSelected)} />
      </div> : null}
  ...
  render() {
    return (
      <div>
        ...
        <ul>
          {!!this.state.selectedFile && this.props.maps.idList.map((idMap, index) => {
            console.log(idMap);
            return (
              <li>
                <p>ID: {idMap}</p>
                <ColorPicker
                  ...
                  color={this.props.maps.textures[idMap]}
